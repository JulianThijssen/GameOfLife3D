...
namespace Life {
    ...
    Matrix4f Matrix4f::operator*(const Matrix4f& m) const {
        Matrix4f dest;
        dest[0] = a[0] * m[0] + a[4] * m[1] + a[8] * m[2] + a[12] * m[3];
        dest[1] = a[1] * m[0] + a[5] * m[1] + a[9] * m[2] + a[13] * m[3];
        dest[2] = a[2] * m[0] + a[6] * m[1] + a[10] * m[2] + a[14] * m[3];
        dest[3] = a[3] * m[0] + a[7] * m[1] + a[11] * m[2] + a[15] * m[3];

        dest[4] = a[0] * m[4] + a[4] * m[5] + a[8] * m[6] + a[12] * m[7];
        dest[5] = a[1] * m[4] + a[5] * m[5] + a[9] * m[6] + a[13] * m[7];
        dest[6] = a[2] * m[4] + a[6] * m[5] + a[10] * m[6] + a[14] * m[7];
        dest[7] = a[3] * m[4] + a[7] * m[5] + a[11] * m[6] + a[15] * m[7];

        dest[8] = a[0] * m[8] + a[4] * m[9] + a[8] * m[10] + a[12] * m[11];
        dest[9] = a[1] * m[8] + a[5] * m[9] + a[9] * m[10] + a[13] * m[11];
        dest[10] = a[2] * m[8] + a[6] * m[9] + a[10] * m[10] + a[14] * m[11];
        dest[11] = a[3] * m[8] + a[7] * m[9] + a[11] * m[10] + a[15] * m[11];

        dest[12] = a[0] * m[12] + a[4] * m[13] + a[8] * m[14] + a[12] * m[15];
        dest[13] = a[1] * m[12] + a[5] * m[13] + a[9] * m[14] + a[13] * m[15];
        dest[14] = a[2] * m[12] + a[6] * m[13] + a[10] * m[14] + a[14] * m[15];
        dest[15] = a[3] * m[12] + a[7] * m[13] + a[11] * m[14] + a[15] * m[15];
